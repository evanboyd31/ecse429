...
So that I can quickly determine the assignments and project tasks that I need to complete.
  ...
  Scenario: Student attempts to filter unfinished todos (Normal Flow)
  ...
  Scenario: Student attempts to filter unfinished todos with additional query params (Alternative Flow)
  ...
  Scenario: Student attempts to filter unfinished todos with incorrect boolean value (Normal Flow)
    ...
    Then the thingifier app should return an empty list of todos
      | faultyBoolean  |
      | False          |
      | 0              |
