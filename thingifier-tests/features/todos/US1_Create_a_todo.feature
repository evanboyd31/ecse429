...
So that I can remember what assignments and project tasks I need to complete.
  ...
  Scenario: Student attempts to create a new todo using Json body (Normal Flow)
  ...
  Scenario: Student attempts to create a new todo using Xml body (Alternative Flow)
  ...
  Scenario: Student attempts to create a new todo without specifying the title (Error Flow)
    When the student sends a POST API request for the "/todos" endpoint with json body containing title "<title>" done status "<doneStatus>" and description "<description>"
