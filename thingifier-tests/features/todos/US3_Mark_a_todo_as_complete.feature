...
So that I can keep track of assignments and project tasks I have already done.
  ...
  Scenario: Student attempts to mark a todo as complete with Json body (Normal Flow)
  ...
  Scenario: Student attempts to mark a todo as complete with xml body (Alternative Flow)
  ...
  Scenario: Student attempts to mark a todo as complete with non-boolean doneStatus (Error Flow)
    When the student sends a POST API request for the "/todos/:id" endpoint with the id of the todo with title "<title>", and with json body containing doneStatus "<doneStatus>"
