...
@when('the student sends a POST API request for the "/todos" endpoint with json body containing title "{title}" done status "{status}" and description "{description}"')
...
@then('the thingifier app should not contain a todo with title "{title}"')
...
@then('the thingifier app should have the todo with title "{title}" marked as done')
def step_then_the_thingifer_app_should_not_contain(context, title):
    todos = fetch_all_todos()
    marked = False
    for todo in todos:
...
def step_then_the_thingifer_app_should_return_a_response_containing_list(context, todosList):
    todos = extract_todo_response(context.response)
...
@then('the thingifier app should return a response containing the unfinished todo with title "{title}" and description "{description}"') 
def step_then_the_thingifer_app_should_return_a_response_containing_unfinished(context, title, description):
    todo = extract_todo_response(context.response)[0]
    assert todo['title'] == title
    assert todo['description'] == description
...
def update_todo(id : str, title : str = None, doneStatus : str = None, description : str = None, isJson : bool = True):
    ...
    if isJson:
        ...
        body = f"""<todo>
            ...
            {f'<doneStatus>{doneStatus}</doneStatus>' if doneStatus else ''}
