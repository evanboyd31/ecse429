...
def create_category(title, description):
    response = httpx.post(categories_url, json={"title": title, "description": description})
    return response
...
@then('a category with title "{title}" and description "{description}" should exist in the thingifier app')
def then_category_with_title_and_description_should_exist(context, title, description):
    response = httpx.get(categories_url)
    categories = response.json()["categories"]
    found = any(category["title"] == title and category["description"] == description for category in categories)
    assert found, f"Category with title '{title}' and description '{description}' not found"
...
def when_create_category_with_id_title_and_description(context, id, title, description):
    response = httpx.post(categories_url, json={"id": id, "title": title, "description": description})
    context.response = response
...
@when('the user attempts to update the category with title "{title}" to have title "{newTitle}" and description "{newDescription}"')
def when_update_category_all_fields(context, title, newTitle, newDescription):
    category_id = get_category_id(title)
    assert category_id is not None, f"Category with title '{title}' not found"
    context.response = httpx.post(f"{categories_url}/{category_id}", json={"title": newTitle, "description": newDescription})

@when('the user attempts to update the category with title "{title}" to have title "{newTitle}"')
def when_update_category_title_only(context, title, newTitle):
    category_id = get_category_id(title)
    assert category_id is not None, f"Category with title '{title}' not found"
    context.response = httpx.post(f"{categories_url}/{category_id}", json={"title": newTitle})
...
@then('the response should contain the updated category with title "{newTitle}" and description "{newDescription}"')
def then_response_contains_updated_category_all_fields(context, newTitle, newDescription):
    response_json = context.response.json()
    print(response_json)
    assert response_json["title"] == newTitle, f"Expected title '{newTitle}', but got '{response_json['title']}'"
    assert response_json["description"] == newDescription, f"Expected description '{newDescription}', but got '{response_json['description']}'"

@then('the response should contain the updated category with title "{newTitle}" and the original description "{description}"')
def then_response_contains_updated_category_title_only(context, newTitle, description):
    response_json = context.response.json()
    assert response_json["title"] == newTitle, f"Expected title '{newTitle}', but got '{response_json['title']}'"
    assert response_json["description"] == description, f"Expected description '{description}', but got '{response_json['description']}'"
...
def then_response_contains_json_array(context):
    response_json = context.response.json()
    assert isinstance(response_json["categories"], list), f"Expected a JSON array, but got {type(response_json['categories'])}"
    assert all(isinstance(category, dict) for category in response_json["categories"]), "Expected all items to be JSON objects"
...
def then_response_contains_xml_array(context):
    ...
    assert isinstance(categories, list), f"Expected an XML array, but got {type(categories)}"
    assert all(isinstance(category, dict) for category in categories), "Expected all items to be XML objects"

@then('the response should contain an empty array')
def then_response_contains_empty_array(context):
    response_json = context.response.json()
    assert response_json["categories"][0]["categories"] == [], f"Expected an empty array, but got {response_json['categories']}"


#-------------------------- VIEW SPECIFIC
    
@when('the user attempts to view a category with the id of the category with title "{title}"')
...
@then('the response should contain the category with title "{title}" and description "{description}"')
def then_response_contains_category(context, title, description):
    response_json = context.response.json()
    assert response_json["categories"][0]["title"] == title, f"Expected title '{title}', but got '{response_json['title']}'"
    assert response_json["categories"][0]["description"] == description, f"Expected description '{description}', but got '{response_json['description']}'"