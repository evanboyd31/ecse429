...
def test_get_project_with_path_id_and_related_query_param_should_get_project_with_that_id_json(
    before_each,
):
    project = test_projects[0]
    # ensure that the response is returns the expected single project
    response = httpx.get(
        f"{projects_url}/{project.get("id")}?title={project.get("title")}"
    )
...
def test_get_project_with_path_id_and_related_query_param_should_get_project_with_that_id_xml(
    before_each,
):
    project = test_projects[0]
    # ensure that the response is returns the expected single project
...
def test_get_project_with_path_id_and_unrelated_query_param_should_get_project_with_that_id_json(
    before_each,
):
    project = test_projects[0]
...
def test_get_project_with_path_id_and_unrelated_query_param_should_get_project_with_that_id_xml(
    before_each,
):
    project = test_projects[0]
    ...
    # ensure that the response is returns the expected single project
    response = httpx.get(
        f"{projects_url}/{project.get("id")}?title=otherTitle", headers=XML_HEADERS
    )
