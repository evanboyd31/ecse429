...
def test_put_project_filter_by_title_json(before_each):
  ...
  response = httpx.put(f"{projects_url}?title={test_project.get("title")}", json=new_data)
...
def test_put_project_filter_by_title_xml(before_each):
  ...
  response = httpx.put(f"{projects_url}?title={test_project.get("title")}", data=xml_data, headers=XML_HEADERS)
