...
def test_put_id_categories_allfields_should_return_categorymodified(setup_module, setup_each):
    ...
    res = httpx.put(categories_url + '/' + test_categories[0]['id'], json=modify_category)
    modify_category.update({"id":test_categories[0]["id"]})
    assert res.status_code == 200
    assert res.json() == modify_category

def test_put_id_categories_titlefield_should_return_categorymodified(setup_module, setup_each):
...
def test_put_id_categories_conflictid_should_return_badrequest(setup_module, setup_each):
...
def test_put_id_categories_nonexistent_should_return_notfound(setup_module, setup_each):
...
def test_put_id_categories_query_conflicting_should_return_badrequest(setup_module, setup_each):
    print("Running test_put_id_categories_query_conflicting_should_return_badrequest")
    modify_category = {"title":"Never seen before title"}
    res = httpx.put(categories_url + '/' + test_categories[0]['id'] + '?id=99999', json=modify_category)
    assert res.status_code == 400

def test_put_id_categories_modifyid_should_return_categorymodified(setup_module, setup_each):
    print("Running test_put_id_categories_query_conflicting_should_return_badrequest")
